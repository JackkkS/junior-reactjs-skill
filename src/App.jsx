...
function App() {
  ...
  useEffect(() =>{
    ...
    let abortContrllor = new AbortController();

    const loadpeke = async () => {
      try {

        setLoading(true);
        let response = await axios.get(`https://pokeapi.co/api/v2/pokemon/${number}`, {
          signal: abortContrllor.signal
        });
    ...
    <div class="max-w-5xl p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
      <div className='grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2'>
        ...
        {loading ? 
          ...
          <>
            <h1>{poke?.name}</h1>
          <button onClick={addFav}>Add to faverite</button>
          <br />
        ...
        <div>
          <h2>Your faverite pokemon</h2>
          {fav.length > 0 ? <FavPoke fav={fav} /> : <div className='flex h-full justify-center items-center'><p>No faverite pokemon</p></div>}
        </div>
      </div>
    </div>
  )
